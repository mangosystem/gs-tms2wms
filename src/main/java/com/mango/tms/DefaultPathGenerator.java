		...
		if (fTG.isOutline()) {
			...
			for (int y = 0; y < tiles.length; y++) {
				for (int x = 0; x < tiles[y].length; x++) {
					...
					String str = "L"+tiles[y][x].getLevel() + " : X" + tiles[y][x].getGridX() + " : Y" + tiles[y][x].getGridY();
	...
	public BufferedImage getTileImage(TileGenerator fTG, Tile tile) {
		...
		try {
			if(fTG.isTileCache()) {
				...
				if(!(cacheFilePath.toLowerCase().endsWith("png") || cacheFilePath.toLowerCase().endsWith("jpg") || cacheFilePath.toLowerCase().endsWith("gif"))) {
